...
parser.add_argument(
    "-d", "--datastore", help="Datastore filename.", default="research_docs_ns.store"
)
parser.add_argument(
    "-s", "--share", action="store_true", help="Creates a public link.", default=False
)
...
def get_ollama_models():
    ...
    for model in result["models"]:
        DBG(
            f"Ollama: {model.model} / {model.details.parameter_size} / {model.details.quantization_level}"
        )
...
with gr.Blocks(theme=theme) as demo_blocks:
    ...
    with gr.Row():
        ...
        val = gr.Slider(
            0, 28, value=8, label="Context size (number of retrieved docs)", step=1.0
        )
        cutoff = gr.Slider(
            0,
            1,
    ...
    with gr.Row():
        model_selector = gr.Dropdown(
            choices=model_names, value=gen_model, label="Choose model"
        )
        tmp = gr.Slider(0.05, 2, value=0.1, label="Temperature", step=0.05)
        npredict = gr.Slider(10, 10000, value=8000, label="Num predict", step=10)
    ...
    def newbot_pipeline(
        ...
        def _cb_stream(chunk):
            nonlocal partial
            partial += chunk.content
            q.put(partial)

        generator.streaming_callback = _cb_stream
    ...
    msg.submit(user, [msg, chatbot], [msg, chatbot], queue=True).then(
        newbot_pipeline,
        [chatbot, val, tmp, cutoff, ignore_extras, npredict, model_selector],
        chatbot,
        queue=True,
    )
